<p>
  <img src="https://capsule-render.vercel.app/api?type=waving&color=20B2AA&height=90"/>
</p>

# Привет!!! <img src = "https://raw.githubusercontent.com/MartinHeinz/MartinHeinz/master/wave.gif" width = 40> я ***Shin Denis***.

<p align=''>
<img src="https://media.giphy.com/media/QvpqTCiEcwtvx6wwJK/giphy.gif" width="200" height="200" frameBorder="0" class="giphy-embed" allowFullScreen>
</p>

## 📊Data Scientist / ML Engineer. 

## 🚀 Обо мне
Люблю превращать сырые данные в понятные инсайты, а идеи — в работающие ML-решения. 

⚙️:
- Провожу анализ данных, ищу закономерности и превращаю их в понятные решения.
- Применяю машинное обучение там, где это действительно даёт ценность.
- Строю модели и думаю не только про качество, но и про применимость.
- Собираю end-to-end пайплайны: от данных до продакшена.
- Автоматизирую процессы, чтобы всё работало стабильно без ручного контроля.
- Настраиваю мониторинг, чтобы модели не «умирали» в проде.

💡:

Всегда за проекты, где можно не просто «потренировать модель», а решить настоящую задачу.

Изучаю и интересуюсь развитием ML, MLOps и созданием масштабируемых решений, которые приносят ценность.

## 🛠 Инструмены:

<table>
  <tr>
    <td><img src="assets/python.svg" width="100"/></td>
    <td><img src="assets/pandas.svg" width="100"/></td>
    <td><img src="assets/numpy.svg" width="100"/></td>
    <td><img src="assets/matplotlib.svg" width="100"/></td>
    <td><img src="assets/scikitlearn.svg" width="100"/></td>
    <td align="center">
      <img src="assets/feature-engine.png" width="100"/><br/>
      Feature Engine
    </td>
  </tr>
  
  <tr>
    <td><img src="assets/mysql.svg" width="100"/></td>
    <td><img src="assets/docker.svg" width="100"/></td>
    <td><img src="assets/circleci.svg" width="100"/></td>
    <td><img src="assets/railway.png" width="100"/></td>
    <td><img src="assets/fastapi.svg" width="100"/></td>
    <td><img src="assets/github.svg" width="100"/></td>
  </tr>
</table>


<p>
  <img src="https://capsule-render.vercel.app/api?type=waving&color=20B2AA&height=90&section=footer"/>
</p>
